...
#### Redis Config
Our Redis extensions can either be instantiated directly or with the help of a `ServerConfig` and `RDevPHPRedisFactory` or `RDevPredisFactory` ([learn more about configs](/app/rdev/configs)).  The config must have the following structure:
```php
[
    "servers" => [
        "master" => [
...
#### Memcached Config
Our Memcached extensions can either be instantiated directly or with the help of a `ServerConfig` and `RDevMemcachedFactory` ([learn more about configs](/app/rdev/configs)).  The config must have the following structure:
```php
[
    "servers" => [
        [
