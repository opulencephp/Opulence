...
class LoginCredentialsTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testGettingLoginToken()
    {
        ...
        $loginToken = new Cryptography\Token(1, $now, $now, true);
    ...
    {
        $userId = 24;
        $token = new Cryptography\Token(1, new \DateTime("now", new \DateTimeZone("utc")),
    ...
    {
        $now = new \DateTime("now", new \DateTimeZone("UTC"));
        $oldLoginToken = new Cryptography\Token(1, $now, $now, true);
        $newLoginToken = new Cryptography\Token(2, $now, $now, true);
