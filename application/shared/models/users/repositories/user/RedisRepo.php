...
class RedisRepo extends Repositories\RedisRepo implements IUserRepo
{
     ...
     */
    public function __construct(Redis\RDevRedis $rDevRedis, Factories\UserFactory $userFactory,
                                Token\ITokenRepo $passwordTokenRepo)
    {
        parent::__construct($rDevRedis);
    ...
    public function add(Users\IUser &$user, Cryptography\Token &$passwordToken, $hashedPassword)
    {
        ...
        $this->rDevRedis->sAdd("users", $user->getId());
        // Create the email index
        $this->rDevRedis->set("users:email:" . strtolower($user->getEmail()), $user->getId());
        // Create the username index
        $this->rDevRedis->set("users:username:" . strtolower($user->getUsername()), $user->getId());
    ...
    {
        return $this->rDevRedis->hGetAll("users:" . $id);
    }
    ...
    {
        return $this->rDevRedis->hSet("users:" . $userId, $hashKey, $value) !== false;
    }
} 